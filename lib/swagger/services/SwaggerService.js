...
var SwaggerService = /** @class */ (function () {
     ...
     *
     */
    SwaggerService.prototype.$afterRoutesInit = function () {
        ...
        if (conf && conf.specPath) {
            var spec = this.getOpenAPISpec();
            Fs.writeFileSync(conf.specPath, JSON.stringify(spec, null, 2));
        }
    };
    SwaggerService.prototype.swaggerValidationErrorHandler = function (error, req, res, next) {
        if (error) {
            var errorOrEmptyObj = error || {};
    ...
    SwaggerService.prototype.$afterControllersInit = function () {
        var _this = this;
        var conf = this.serverSettingsService.get("swagger");
        var host = this.serverSettingsService.getHttpPort();
        var path = conf && conf.path || "/docs";
        if (conf) {
            ...
            if (conf.validate && conf.specPath && Fs.existsSync(conf.specPath)) {
                return new Promise(function (resolve, reject) {
                    return _this.validateMiddleware()(conf.specPath, _this.expressApplication, function (err, middleware) {
                        if (err) {
                        ...
                        _this.expressApplication.use(middleware.metadata(), middleware.files(), middleware.parseRequest(), middleware.validateRequest())
                            .use(_this.swaggerValidationErrorHandler);
                        resolve();
                    });
                });
            }
